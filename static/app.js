...
(function () {
  ...
  const GAUGES = {
    ...
    // Manual transmission — sends selected_gear (integer 0=Neutral, 1–6) to the backend
    gear: { min: 0, max: 6, unit: "GEAR", param: "selected_gear", key: "selected_gear" },
  };
  ...
  const STATUS_RANGES = {
    ...
    // Selected Gear — badge shows the gear name, always "optimal"; 0 = Neutral
    gear: [
      { max: 0, status: "optimal", label: "NEUTRAL" },
      { max: 1, status: "optimal", label: "1ST" },
      { max: 2, status: "optimal", label: "2ND" },
      { max: 3, status: "optimal", label: "3RD" },
      { max: 4, status: "optimal", label: "4TH" },
      { max: 5, status: "optimal", label: "5TH" },
      { max: Infinity, status: "optimal", label: "6TH" },
    ],
    tire_fl: [
      { max: 25, status: "danger", label: "LOW DANGER" },
      { max: 30, status: "warning", label: "UNDER-INFLATED" },
  ...
  function setGaugeValue(type, value) {
    ...
    if (num) {
      if (type === "gear" && clamped === 0) {
        // Display "N" for Neutral instead of animating to 0
        num.textContent = "N";
      } else {
  ...
  function connect() {
    ...
    ws.onmessage = (e) => {
      ...
      if (msg.type === "telemetry" && msg.data) {
        ...
        // Selected Gear: update live gauge with what the server echoes back
        if (msg.data.selected_gear != null) {
          setGaugeValue("gear", msg.data.selected_gear);
        }
        ...
        addFeedEntry(
          ...
          `GEAR <span style="color:var(--purple)">${msg.data.selected_gear === 0 ? "N" : (msg.data.selected_gear ?? "N")}</span>`,
          "data"
        );
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    $$("input[type='range']").forEach((slider) => {
      ...
      slider.addEventListener("input", () => {
        ...
        if (display && !display.querySelector("input")) {
          const val = parseFloat(slider.value);
          if (slider.dataset.gauge === "gear" && val === 0) {
            display.textContent = `N ${cfg.unit}`;
          } else {
    ...
    setInterval(() => {
      ...
      const targets = {
        ...
        battery: 14.2,
        oil: 40,
        // Auto-drive selects a sensible gear based on speed phase; 0 = Neutral when idling
        gear: tSpeed >= 100 ? 5 : tSpeed >= 60 ? 4 : tSpeed >= 40 ? 3 : tSpeed >= 20 ? 2 : tSpeed > 0 ? 1 : 0,
      };
  ...
  // --- GEAR UI OVERRIDE (NEUTRAL STATE FORMATTING) ---
  const gearObserver = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      if (mutation.type === 'characterData' || mutation.type === 'childList') {
        const text = mutation.target.textContent;
        // Fix the live gauge number
        if (mutation.target.classList && mutation.target.classList.contains('gauge-number') && text === "0") {
          mutation.target.textContent = "N";
        }
        // Fix the slider text
        if (mutation.target.classList && mutation.target.classList.contains('slider-value-display') && text.trim() === "0 GEAR") {
          mutation.target.textContent = "N GEAR";
        }
      }
    });
  });

  // Attach observer once DOM is fully loaded
  document.addEventListener("DOMContentLoaded", () => {
    const gearCard = document.querySelector('.gauge-card.gear');
    if (gearCard) {
      gearObserver.observe(gearCard, {
        childList: true,
        characterData: true,
        subtree: true
      });
