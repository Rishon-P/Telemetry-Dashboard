...
(function () {
  ...
  /* ── Layer 3 flicker guard ─────────────────────── */
  // Stores the last report string rendered into the Layer 3 panel.
  // The animation reflow (forced layout + class toggle) only fires when
  // this value changes — not on every WebSocket message.
  let _lastLayer3Report = null;
  ...
  function updateLayer3Panel(report, rootCause, gatewayAlerts) {
    ...
    if (report) {
      // Detect whether the AI text has actually changed since the last render.
      // The WebSocket loop fires at high frequency; without this guard every
      // message triggers a forced reflow + CSS class toggle which causes the
      // panel to flash on every tick even when the text is identical.
      const reportChanged = report !== _lastLayer3Report;
      ...
      // Only restart the entry animation when the content genuinely changes.
      // Skipping the forced reflow on unchanged messages eliminates the flicker.
      if (reportChanged) {
        _lastLayer3Report = report;
        panel.classList.remove("layer3-animate");
        void panel.offsetWidth; // reflow required to reset the CSS animation
        panel.classList.add("layer3-animate");
      }
    } else {
      _lastLayer3Report = null;
